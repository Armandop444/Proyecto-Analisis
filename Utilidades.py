    ...
    if cuantasRaices>0:
        while cuantasRaices>0:
            ...
            while bandera == 0:
                ...
                if parentesisA==0:
                    salida=salida+valor[inicioBuscar:posicionInicio]+" "+valor[posicionInicio:(posicion+1)]+" "
                    bandera=1
        ...
        while bandera == 0:
            ...
            if parentesis==0:
                if inicioBuscar==posicionInicio and original.count("raiz")<=1:
                    salida=salida+convertido[posicionConvertido]+original[(posicion+1):]
                else:
                    salida=salida+original[inicioBuscar:posicionInicio]+convertido[posicionConvertido]
                bandera=1
                inicioBuscar=posicion+1
                ...
                for k in range(len(indice)):
                    if indice[k] != ")":
                        if indice[k].isdigit() == True:
                            ...
                            raise MathError(f"Error en funcion raiz({dato}) Solo indices naturales mayor o igual a 1")
